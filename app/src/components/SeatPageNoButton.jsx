...
const SeatPageNoButton = (selectedDate, selectedTime) => {
  ...
  useEffect(() => {
    const fetchSeats = async () => {
      try {
        // Make API call with selectedDate and selectedTime
        // const response = await fetch(`/api/seatingPlan?date=${selectedDate}&time=${selectedTime}`);
        const response = await fetch('seatData.json');
        const data = await response.json();
        setSeatData(data);
        setShowRestOfPage(true);
  ...
  //   //   conditions to display seat map
  //   if (!selectedDate || !selectedTime) {
  //     return <div>Please select a date and time.</div>;
  //   }
...
export default SeatPageNoButton;
